...
RUN curl -s https://pkg.cfssl.org/R1.1/cfssl_linux-amd64 -o /usr/bin/cfssl && chmod +x /usr/bin/cfssl \
  ...
  && curl -s -L https://storage.googleapis.com/kubernetes-release/release/v1.5.2/bin/linux/amd64/kubectl -o /usr/bin/kubectl && chmod +x /usr/bin/kubectl \
  ...
  && curl -s -L https://github.com/UKHomeOffice/kd/releases/download/v0.2.3/kd_linux_amd64 -o /usr/bin/kd && chmod +x /usr/bin/kd
